...
def run_epoch(real_lr,session, keep_prob,log_file,batch_size, model, data, eval_op,step_index,summary_writer,istraining=True):
    ...
    for iter in range(iterations):
        ...
        if istraining:
            ...
            if iter % 10 == 1:
                info = "%.3f: acc(1): %.4f acc(5): %.4f loss %.4f speed: %.3f sec lr %.5f" % (iter * 1.0 / iterations, correct_top_1 * 1.0 / batch_size, correct_top_5 * 1.0 / batch_size, cur_short_term_loss,  
            ...
            if iter % 100 == 1:
                info = "%.3f: acc(1): %.4f acc(5): %.4f loss %.4lf speed: %.3f sec lr %.5f" % (iter * 1.0 / iterations, correct_top_1 * 1.0 / batch_size, correct_top_5 * 1.0 / batch_size, total_loss * 1.0 /(iter + 1),
                   (time.time() - start_time) * 1.0 / ((iter + 1) * batch_size) ,lr )
                print (info)
                log_file.write(info + '\n')
                log_file.flush()
    ...
    with tf.Graph().as_default(), tf.Session(config=Session_config) as sess:    
        with tf.device('/gpu:1'):
            ...
            for i in range(last_epoch + 1,train_config.max_epoch):
                ...
                info = "Epoch: %d Train accury(top 1): %.4f Train accury(top 5): %.4f Loss %.4f" % (i, train_accury_1,train_accury_5,train_loss)
                print (info)
                fout.write(info + '\n')
                fout.flush()
