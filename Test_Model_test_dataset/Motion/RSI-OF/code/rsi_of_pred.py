...
def run_epoch(session, keep_prob,log_file,batch_size, model, data, eval_op,step_index,summary_writer,istraining=True):
    ...
    for iter in range(iterations):
        fetches = [model.fc6l, model.logits,model.correct_num_top_1, model.correct_num_top_5, model.loss,eval_op] 
        ...
        batch_fc6, batch_logits,correct_top_1, correct_top_5, loss, _ = session.run(fetches, feed_dict)

        model_fc6_list.append(batch_fc6)
    ...
    print ('data saving...')
    model_fc6_tuple = tuple(model_fc6_list)
    model_logits_tuple = tuple(model_logits_list)
    model_fc6 = np.concatenate(model_fc6_tuple,0)
    model_logits =  np.concatenate(model_logits_tuple,0)
    
    np.save('logits.npy',model_logits)
    print ('data saved!')
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--file_path_test',
      type=str,
      default='../labels/test_label_len',
      help='file_path is the path of [video_path label] file.'
  )
  parser.add_argument(
      '--data_root_dir',
      type=str,
      default='/home/share2/chaLearn-Iso/RSI_flow/',
